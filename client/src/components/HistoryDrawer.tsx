...
const TYPE_ICON: Record<string, React.ReactNode> = {
  text:  <FileText size={18} className="text-indigo-500" />,
  audio: <Mic size={18} className="text-amber-500" />,
  video: <Video size={18} className="text-rose-500" />,
  ocr:   <ImageIcon size={18} className="text-emerald-500" />,
};
    ...
    <>
      ...
      <div className={`fixed top-0 right-0 h-full w-full max-w-sm z-50 transition-transform duration-200 ease-out ${
        ...
        <div className={`px-5 py-4 flex items-center justify-between border-b ${border}`}>
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className={`text-xs mt-0.5 ${muted}`}>{total} saved translations</p>
            </div>
          </div>
        ...
        <div className="flex-1 overflow-y-auto p-3 space-y-1 custom-scrollbar">
          {loading ? (
            <p className={`text-sm text-center py-14 ${muted} animate-pulse`}>Loading…</p>
          ) : records.length === 0 ? (
            <div className="flex flex-col items-center justify-center py-14 gap-3">
              <Clock size={36} className={muted} />
              <p className={`text-sm text-center ${muted}`}>No past translations yet</p>
            </div>
          ) : records.map(rec => (
            <button
              key={rec.id}
            ...
            >
              <div className={`h-11 w-11 rounded-xl flex items-center justify-center shrink-0 ${
                ...
                {TYPE_ICON[rec.input_type] || <FileText size={18} />}
              </div>
              <div className="flex-1 min-w-0">
                <p className="text-sm font-semibold truncate">
                  {rec.file_name || (rec.original_text?.substring(0, 30) + '…') || 'Text Input'}
                </p>
                <p className={`text-xs mt-0.5 ${muted}`}>
                  {TYPE_LABEL[rec.input_type] || rec.input_type} → {rec.target_language} · {fmt(rec.created_at)}
                </p>
              </div>
